...
%token MAIN
%token NODE_TEST_FUNC
%token NODE_PRINTF
%token NODE_POW_FUNC     
%token NODE_SQRT_FUNC
...
var_decl: VAR_TYPE[type] VAR_NAME[name]
        ...
        | error VAR_NAME[name] "=" expr
            { exit_with_error(INIT_INVALID_TYPE, parser_line); }
        ;
...
func: NODE_TEST_FUNC { $$ = TEST_FUNC; }
    | NODE_PRINTF    { $$ = PRINTF; }
    | NODE_POW_FUNC  { $$ = POW_FUNC; }    
    | NODE_SQRT_FUNC { $$ = SQRT_FUNC; }
    ;
